<template>
  <el-form ref="formRef" :model="form" label-width="120px">
    <!-- 商品名称 -->
    <el-form-item label="电影名称" prop="name" style="width: 92%">
      <el-input v-model="form.name" placeholder="请填写电影名称" />
    </el-form-item>
    <!-- 商品分类 -->
...
const loadGoods = async () => {
  if (form.id) {
    ...
    if (goods.picture !== "") {
      const fileName = goods.picture.substring(
        goods.picture.lastIndexOf("/") + 1
      );
      if (fileName) {
